        ...
        public InventoryController(IUnitOfWork unitOfWork)
    {
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                // Show newest items first so v?a t?o s? xu?t hi?n ngay d?u danh sách
            ...
            {
                return StatusCode(500, new { message = "L?i khi t?i danh sách hàng t?n kho", error = ex.Message });
            }
        }
        ...
        public async Task<ActionResult<object>> GetInventoryItem(int id)
        {
            ...
            {
                return StatusCode(500, new { message = "L?i khi t?i thông tin s?n ph?m", error = ex.Message });
            }
        }
        ...
        public async Task<ActionResult<object>> CreateInventoryItem([FromBody] InventoryItemDto dto)
        {
            try
            {
                ...
                {
                    return BadRequest(new { message = "Mã s?n ph?m dã t?n t?i" });
                }
                ...
                return CreatedAtAction(nameof(GetInventoryItem), new { id = item.ItemId }, 
                    new { message = "T?o s?n ph?m thành công", itemId = item.ItemId, totalItems = newTotal });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<object>> UpdateInventoryItem(int id, [FromBody] InventoryItemDto dto)
        {
            try
            {
                ...
                return Ok(new { message = "C?p nh?t s?n ph?m thành công", itemId = item.ItemId });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<object>> CreateStockMovement(int id, [FromBody] StockMovementDto dto)
        {
            try
            {
                ...
                if (newStock < 0)
                {
                    return BadRequest(new { message = "S? lu?ng t?n kho không th? âm" });
                }
        ...
        public async Task<ActionResult<object>> SoftDeleteItem(int id)
        {
            try
            {
                ...
                if (!item.IsActive)
                {
                    return Ok(new { message = "S?n ph?m dã ? tr?ng thái ?n", itemId = id });
                }
                ...
                return Ok(new { message = "Ðã ?n s?n ph?m", itemId = id });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "L?i khi xóa s?n ph?m", error = ex.Message });
            }
        }
        ...
        public async Task<ActionResult<object>> ToggleActive(int id)
        {
            try
            {
                ...
                await LogAuditAsync("InventoryItem", item.ItemId, "ToggleActive", oldValues,
                    System.Text.Json.JsonSerializer.Serialize(item));
                return Ok(new { message = "Ðã c?p nh?t tr?ng thái", itemId = id, isActive = item.IsActive });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<object>> GetSuppliers()
        {
            ...
            {
                return StatusCode(500, new { message = "L?i khi t?i nhà cung c?p", error = ex.Message });
            }
        }
        ...
        public async Task<ActionResult<object>> GetDashboardStats()
        {
            try
            {
                ...
                // SQLite không h? tr? Sum trên bi?u th?c decimal ph?c t?p -> chuy?n sang client
            ...
            {
                return StatusCode(500, new { message = "L?i khi t?i th?ng kê", error = ex.Message });
            }
        }
        ...
        public async Task<ActionResult<object>> GetLowStockAlerts()
        {
            ...
            {
                return StatusCode(500, new { message = "L?i khi t?i c?nh báo t?n kho", error = ex.Message });
            }
        }
