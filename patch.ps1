$path = "d:\quanlyresort-main\quanlyresort-main\QuanLyResort\wwwroot\customer\room-detail.html"
$content = Get-Content -Raw -Encoding UTF8 $path

$content = $content -replace '<script src="js/customer-api.js"></script>', "<script src=`"js/customer-api.js`"></script>`r`n  <script src=`"js/simple-payment.js`"></script>"
...
$content = $content.Replace('const urlParams = new URLSearchParams(window.location.search);', $toastCode)

$submitBookingOld = [regex]::Match($content, '(?s)async function submitBooking\(room\) \{.*?hidePageLoading\(\);\s*\}\s*\}').Value
$submitBookingNew = @'
...
if ($submitBookingOld -and $submitBookingOld.Length -gt 100) {
    $content = $content.Replace($submitBookingOld, $submitBookingNew)
} else {
    Write-Error "Could not find submitBooking block!"
    exit 1
}
...
if (-not $content.Contains("simplePaymentModal")) {
    $content = $content.Replace("</body>", $modalHtml)
}

Set-Content -Path $path -Value $content -Encoding UTF8
Write-Output "Successfully patched room-detail.html"
